"""
Motor de Jugadores - FBref Downloader
"""
...
def clean_columns(df):
    """Aplana MultiIndex y limpia nombres"""
    new_cols = []
    for col in df.columns:
        if isinstance(col, tuple):
            # Coger el nivel más específico que no esté vacío
...
def download_player_stats():
    print("📥 Iniciando descarga de JUGADORES (FBref)...")
    
    try:
        # Descargar dos temporadas para tener contexto
        fbref = sd.FBref(leagues="ESP-La Liga", seasons=["2425", "2526"])
        
        # 1. Calendario (Para fechas)
        print("📅 Descargando Calendario...")
        schedule = fbref.read_schedule().reset_index()
        schedule = clean_columns(schedule)
        
        # Buscar columna ID de partido
        id_col = 'game_id' if 'game_id' in schedule.columns else 'game'
        if 'date' not in schedule.columns:
            print("⚠️ Error: No se encuentra columna fecha en calendario")
            return

        schedule_min = schedule[[id_col, 'date']].drop_duplicates()

        # 2. Stats
        print("⚽ Descargando Stats (Summary + Misc)...")
        ...
        # Nos aseguramos de tener estas claves
        join_keys = [k for k in join_keys if k in summary.columns]
        
        # Columnas extra de Misc (Faltas, Tarjetas)
        misc_cols = [c for c in ['fls', 'fld', 'crdy', 'crdr', 'off'] if c in misc.columns]
